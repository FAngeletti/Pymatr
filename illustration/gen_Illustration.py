...
__latexFile__=open('Illustration.tex', 'w+')
...
__latex__(r'''\documentclass{article}
...
seed=random.seed() 
__latex__(r'''
% Random seed used to generate the text file: ''')
__pynclusion__(seed)
__latex__(r'''
...
for (level, set ) in enumerate(Us): 
		__newContext__()
		__latex__(r''' U_{''')
		__pynclusion__(level)
		__latex__(r'''} &= ''')
		__pynclusion__(pr.nset(2, set))
		__latex__(r''' \\ ''')
		__popContext__()
...
\begin{equation}
\mEx = ''')
__pynclusion__(pr.mat(E,spacing="0.1cm"))
__latex__(r'''
\end{equation}
...
for c, (lbd, left, right) in enumerate(eigens):
	...
	__pynclusion__(c)
	__latex__(r'''} = ''')
	__pynclusion__(sym.latex(lbd))
	__latex__(r''',  \quad& \vEig{''')
	__pynclusion__(c)
	__latex__(r'''} = ''')
	__pynclusion__(pr.mat(right))
	__latex__(r''', \quad& \fEig{''')
	__pynclusion__(c)
	__latex__(r'''} = ''')
	__pynclusion__(pr.mat(left))
	__latex__(r''' \\ ''')
	__popContext__()
...
\begin{equation}
\ex\mSl = ''')
__pynclusion__(pr.mat(lshadow,spacing="0.1cm"))
__latex__(r'''.
\end{equation}
...
\begin{equation}
\ex\mAr= ''')
__pynclusion__(pr.mat(Ar))
__latex__(r''' ,\quad  \ex\mEr = ''')
__pynclusion__(pr.mat(Er, spacing="0.1cm"))
__latex__(r'''
\end{equation}
...
for (k,(path, prob))  in enumerate(zip(paths, probP)): 
	...
	__pynclusion__(k+1)
	__latex__(r'''} =''')
	__pynclusion__(pr.nset(1,path))
	__latex__(r''', \quad &p\p{\pC_{''')
	__pynclusion__(k+1)
	__latex__(r'''}} =''')
	__pynclusion__(prob)
	__latex__(r''' ''')
...
empirical histogram computed from $''')
__pynclusion__(nhist)
__latex__(r'''$ realizations of $\vec X$ with $N=''')
__pynclusion__(nsyn)
__latex__(r'''$.}
\end{figure}
